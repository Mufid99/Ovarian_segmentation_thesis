...
from nnunet.inference.segmentation_export import get_segmentation_nifti, save_segmentation_nifti_from_softmax, save_segmentation_nifti
...
# Created by Mufid
# preprocess for single image
def preprocess_image(input, trainer, segs_from_prev_stage=None):
    segs_from_prev_stage = [None]
    d, _, dct = trainer.preprocess_patient_single(input)
    return d, dct
...
# Created by Mufid
# Predicts segmentation for single tensor
def predict_case(input, folds, trainer, params, ensemble = False, segs_from_prev_stage=None, do_tta=True, mixed_precision=True,
    ...
    d, dct = preprocess_image(input, trainer)
    

    # softmax values for each of the labels (in current case 0, 1, 2)
    all_softmax_outputs = np.zeros((len(params), trainer.num_classes, *d.shape[1:]), dtype=np.float16)
    # the chosen label for each pixel
    all_seg_outputs = np.zeros((len(params), *d.shape[1:]), dtype=int)
    ...
    if ensemble: 
        ...
        trainer.load_checkpoint_ram(params[2], False)
        res = trainer.predict_preprocessed_data_return_seg_and_softmax(d, do_mirroring=do_tta,
                                                                            mirror_axes=trainer.data_aug_params['mirror_axes'],
                                                                            use_sliding_window=True,
                                                                            step_size=step_size, use_gaussian=True,
    ...
    # attribute to check for predict fastest to work
    if hasattr(trainer, 'regions_class_order'):
        region_class_order = trainer.regions_class_order
    else:
        region_class_order = None
    ...
    return get_segmentation_nifti(seg, dct, 0, None)
