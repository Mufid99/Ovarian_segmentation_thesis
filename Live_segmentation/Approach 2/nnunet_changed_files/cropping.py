...
# Created by Mufid
def load_case_from_single_file(input):
    properties = OrderedDict()
    data_itk = sitk.GetImageFromArray(input[0])

    properties["original_size_of_raw_data"] = np.array(data_itk.GetSize())[[2, 1, 0]]
    properties["original_spacing"] = np.array([999., 1., 1.])
    properties["input"] = input

    properties["itk_origin"] = data_itk.GetOrigin()
    properties["itk_spacing"] = (1., 1., 999.)
    properties["itk_direction"] = data_itk.GetDirection()
    return input.astype(np.float32), None, properties
...
class ImageCropper(object):
    ...
    # Created by Mufid
    @staticmethod
    def crop_from_single_file(input):
        data, seg, properties = load_case_from_single_file(input)
        return ImageCropper.crop(data, properties, seg)
