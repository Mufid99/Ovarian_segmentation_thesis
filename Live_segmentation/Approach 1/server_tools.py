...
def human_keys(text):
    ...
    # if (datetime.now() - lastActiveCheck).seconds > ACTIVE_CHECK_PERIOD:
    #     # loop over all previously active devices
    #     for (client_name, ts) in list(lastActive.items()):
    #         if (datetime.now() - lastActiveCheck).seconds > ACTIVE_CHECK_PERIOD:
    #             print("[INFO] lost connection to {}".format(client_name))
    #             lastActive.pop(client_name)
    #             frameDict.pop(client_name)
    ...
    #     break    
    # if image_hub.zmq_socket.poll(2000, zmq.POLLIN):
    #     print("got message")
    #     client_name, frame = image_hub.recv_image()
    # else:
    #     print("Video stream ended from client")
